...
- 🗺️ **土地（国）をタップ** → そこに棲むいきものの逆引き一覧。
- 🌿 **環境（biome）でしぼる** … サバンナ/森林/熱帯雨林/草原/寒帯林/高山/乾燥地/湿地/海/極地。
- 🔍 **検索**（和名・学名・分類・環境）／ 🎲 **探検モード**（ランダムに1種へ旅）／ 🧭 **図鑑コンプ率**（端末に保存）。
- 図鑑カード：図鑑No.・和名・学名・分類・**レア度（IUCN保全状況の色）**・ステータス・解説・実写真・生息地。

レア度＝IUCN保全状況：`LC` コモン → `NT` アンコモン → `VU` レア → `EN` スーパーレア → `CR` レジェンダリー。

## データ
- サンプル **28種**（実在のおおよそ正しい分布・ステータス）。
