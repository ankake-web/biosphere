...
function onAuth(ev, session){   // ログイン状態が変わったらUIを更新＋同期をトリガ（local-firstの背景同期）
  ...
  if(authUser && (ev==='SIGNED_IN'||ev==='INITIAL_SESSION')){
    ...
    pushAllSightings();   // コミットC：ローカル記録をクラウドへ（冪等）。D で pull/移行/state を足す
  }
...
addEventListener('online',()=>{ if(authUser) pushAllSightings(); });                          // ネット復帰で未同期を送る
addEventListener('visibilitychange',()=>{ if(authUser&&document.visibilityState==='visible') pushAllSightings(); });   // タブ復帰でも
