...
const USER = (()=>{
  ...
  return {
    ...
    // ── Stage1b 同期用 seam（SYNC が list を触った後は必ずここ経由。localStorage 直書きは禁止＝in-memory list が stale 化する）──
    persist(){ return save(); },
    ensureCids(){ let ch=false; for(const r of list){ if(!r.cid){ r.cid=genCid(); ch=true; } } return ch?save():true; },   // 既存記録に端末安定ID(cid)を1回だけ付与（ログイン時のbackfill・既存cidは触らない）
    replaceAll(nl){ list.length=0; for(const r of (nl||[])) list.push(r); return save(); },   // pull/mergeの結果でlistを更新＋保存
    hydrateFromLS(){ try{ const n=JSON.parse(localStorage.getItem(LS)||'[]'); if(Array.isArray(n)){ list.length=0; for(const r of n) list.push(r); } }catch(e){} },   // 他タブの更新を取り込む（saveしない＝ping-pong防止）
  };
})();
...
function submitSeen(id){
  ...
  updateSeenBtn(id);
  pushSighting(rec);   // Stage1b：ログイン中ならクラウドへ（fire-and-forget・ゲストはno-op）
...
function saveQuests(){ try{ localStorage.setItem('biosphere_quests',JSON.stringify([...QDONE])); }catch(e){} if(typeof scheduleStatePush==='function') scheduleStatePush(); }
...
function sbClient(){   // クライアントを遅延生成（PKCE固定・detectSessionInUrlで ?code= を交換）。★promise単一化＝並行呼び出しでもcreateClient/リスナは1回だけ
  return _sbClientP || (_sbClientP=(async()=>{
    const lib=await _loadSb();
    const c=lib.createClient(SB_URL, SB_KEY, { auth:{
      flowType:'pkce', detectSessionInUrl:true, persistSession:true, autoRefreshToken:true, storageKey:SB_STORAGE_KEY } });
    try{ c.auth.onAuthStateChange((ev,session)=>onAuth(ev,session)); }catch(e){}
    return c;
  })());
}
function accountName(){ const u=authUser; if(!u)return ''; const m=u.user_metadata||{}; return m.full_name||m.name||u.email||''; }   // UI表示用（メールfallback可）
function profileName(){ const m=(authUser&&authUser.user_metadata)||{}; return m.full_name||m.name||null; }   // DBへ書く用＝メール(PII)は入れない
function onAuth(ev, session){   // ログイン状態が変わったらUIを更新＋同期をトリガ（local-firstの背景同期）
  const prev=authUser&&authUser.id;
  authUser=(session&&session.user)||null;
  updateAuthUI();
  if(authUser && (ev==='SIGNED_IN'||ev==='INITIAL_SESSION')){
    if(ev==='SIGNED_IN' && authUser.id!==prev) toast('☁️',(esc(accountName())||'アカウント')+' でログインしました',2600);
    reconcileOwner(authUser.id);   // ★別アカウントに切替なら前ユーザーの個人データを隔離してから同期（クラウド越境を防ぐ）
    migrateAndPull();   // 初回ログイン＝ローカル→クラウドpush→クラウド→ローカルunion（別端末で消えない）
  }
  if(ev==='SIGNED_OUT'){ authUser=null; _signedCache.clear(); updateAuthUI(); }   // ローカルは残す（同一ユーザー再ログインで復帰）。別アカ時は次ログインのreconcileOwnerが隔離。署名URLキャッシュは破棄
}
// この端末のローカル個人データが「どのアカウントのものか」を追跡し、別アカウントへ流入させない
function reconcileOwner(uid){
  let owner=null; try{ owner=localStorage.getItem('biosphere_owner'); }catch(e){}
  if(owner && owner!==uid) isolateLocalUserData();   // 前オーナー≠今のuid＝別アカウント切替→前ユーザーの個人データを隔離
  try{ localStorage.setItem('biosphere_owner',uid); }catch(e){}   // owner未設定(純ゲスト)なら、このデータは正当にこのユーザーへ移行
}
function isolateLocalUserData(){   // 別アカウント切替時：この端末の個人データをクリア＝前ユーザーの記録を新アカウントのクラウドへpushしない
  try{ USER.replaceAll([]); }catch(e){}
  SEEN.clear(); saveSeen(); QDONE.clear(); saveQuests(); BADGES.clear(); saveBadges();
  SYNCED.clear(); saveSynced(); STATESYNCED.clear(); saveStateSynced(); _signedCache.clear();
...
// ===== クラウド同期（local-first・コミットC=記録push／D=pull・移行／E=写真）=====
// 真実源は常に localStorage。ログイン中だけ裏で Supabase へ同期。★ゲスト/未ログインは全て即return＝既存挙動・オフラインは不変。
function genCid(){ try{ if(window.crypto&&crypto.randomUUID) return crypto.randomUUID(); }catch(e){} return 'c-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,10); }   // 端末安定ID（非セキュアコンテキストでも落ちないフォールバック）
const SYNCED=(()=>{ try{ return new Set(JSON.parse(localStorage.getItem('biosphere_synced')||'[]')); }catch(e){ return new Set(); } })();   // cloudへ確定送信済みの cid（outbox＝未同期の把握用）
function saveSynced(){ try{ localStorage.setItem('biosphere_synced',JSON.stringify([...SYNCED])); }catch(e){} }
const STATESYNCED=(()=>{ try{ return new Set(JSON.parse(localStorage.getItem('biosphere_statesynced')||'[]')); }catch(e){ return new Set(); } })();   // push済み状態id（'s:種'/'q:クエスト'/'b:バッジ'）＝状態を差分送信（全件再送しない）
function saveStateSynced(){ try{ localStorage.setItem('biosphere_statesynced',JSON.stringify([...STATESYNCED])); }catch(e){} }
function sightingRow(rec){ return { user_id:authUser.id, client_id:rec.cid, species_id:rec.id, species_sci:rec.sci||null,
  seen_at:new Date(rec.at||Date.now()).toISOString(), lat_coarse:(typeof rec.lat==='number'?rec.lat:null), lng_coarse:(typeof rec.lng==='number'?rec.lng:null),
  place_label:rec.place||null, confidence:rec.confidence||null, note:rec.note||null, source:rec.source||'manual', photo_path:rec.photo_path||null }; }
let _pushing=false;
async function pushSighting(rec){   // 1件をクラウドへ（fire-and-forget・冪等upsert）。ゲスト/未ログインは即no-op
  if(!authUser || !rec) return;
  try{
    if(!rec.cid){ rec.cid=genCid(); if(!USER.persist()) return; }   // cidを永続できなければpushしない（quota時の別cid重複行を防ぐ）
    const c=await sbClient();
    const { error }=await c.from('sightings').upsert(sightingRow(rec), { onConflict:'user_id,client_id' });
    if(!error){ SYNCED.add(rec.cid); saveSynced(); if(rec.photo && !rec.photo_path) uploadPhoto(rec); }   // 写真は行upsert後に別途アップ（コミットE・落ちてもテキストは同期済み）
  }catch(e){}
}
async function pushAllSightings(){   // 未同期のローカル記録を全部push（ログイン時・online復帰・タブ復帰）。冪等（unique(user_id,client_id)+upsert）
  if(!authUser || _pushing) return; _pushing=true;
  try{
    if(!USER.ensureCids()){ return; }   // 既存記録にcidをbackfill（1回だけ）。永続失敗(quota)なら未永続cidでpushせず次の機会へ委ねる
    const c=await sbClient();
    const pending=USER.all().filter(r=>r.cid && !SYNCED.has(r.cid));
    for(let i=0;i<pending.length;i+=100){
      const batch=pending.slice(i,i+100);
      const { error }=await c.from('sightings').upsert(batch.map(sightingRow), { onConflict:'user_id,client_id' });
      if(error) break;   // 失敗＝未同期のまま残し次の機会(online/visibility/再ログイン)に再送
      batch.forEach(r=>SYNCED.add(r.cid)); saveSynced();
    }
  }catch(e){} finally{ _pushing=false; }
}
addEventListener('online',()=>{ if(authUser){ pushAllSightings(); syncPull(); } });                          // ネット復帰＝未同期を送り最新を取り込む
addEventListener('visibilitychange',()=>{ if(authUser&&document.visibilityState==='visible'){ pushAllSightings(); syncPull(); } });   // タブ復帰でも
...
let _syncingState=false, _migrateP=null, _pulling=false;
function saveSeen(){ try{ localStorage.setItem('biosphere_seen',JSON.stringify([...SEEN])); }catch(e){} }
function rowToRec(row){ return { id:row.species_id, sci:row.species_sci||undefined, at:row.seen_at?new Date(row.seen_at).getTime():Date.now(),
  lat:(typeof row.lat_coarse==='number'?row.lat_coarse:null), lng:(typeof row.lng_coarse==='number'?row.lng_coarse:null),
  place:row.place_label||null, note:row.note||null, confidence:row.confidence||null, photo:null, photo_path:row.photo_path||null, cid:row.client_id }; }
let _statePushT=null;
function scheduleStatePush(){ if(!authUser||_syncingState) return; clearTimeout(_statePushT); _statePushT=setTimeout(pushState,1200); }   // 状態変化をまとめてpush（pull中はno-op＝フィードバック防止）
async function pushState(){   // ローカルの seen/quests/badges＋profile をクラウドへ。★差分のみ（STATESYNCED未登録だけ）＝全件再送しない
  if(!authUser) return;
  try{ const c=await sbClient(), uid=authUser.id, jobs=[], mark=[];
    const nseen=[...SEEN].filter(x=>!STATESYNCED.has('s:'+x));
    const nq=[...QDONE].filter(x=>!STATESYNCED.has('q:'+x));
    const nb=[...BADGES].filter(x=>!STATESYNCED.has('b:'+x));
    if(nseen.length) jobs.push(c.from('user_seen').upsert(nseen.map(species_id=>({user_id:uid,species_id})),{onConflict:'user_id,species_id',ignoreDuplicates:true}).then(({error})=>{ if(!error) nseen.forEach(x=>mark.push('s:'+x)); }));
    if(nq.length)    jobs.push(c.from('user_quests').upsert(nq.map(quest_id=>({user_id:uid,quest_id})),{onConflict:'user_id,quest_id',ignoreDuplicates:true}).then(({error})=>{ if(!error) nq.forEach(x=>mark.push('q:'+x)); }));
    if(nb.length)    jobs.push(c.from('user_badges').upsert(nb.map(badge_id=>({user_id:uid,badge_id})),{onConflict:'user_id,badge_id',ignoreDuplicates:true}).then(({error})=>{ if(!error) nb.forEach(x=>mark.push('b:'+x)); }));
    jobs.push(c.from('profiles').upsert({id:uid,display_name:profileName(),updated_at:new Date().toISOString()},{onConflict:'id'}));
    await Promise.all(jobs);
    if(mark.length){ mark.forEach(x=>STATESYNCED.add(x)); saveStateSynced(); }
  }catch(e){}
}
async function syncPull(){   // cloud→local を union（欠けを足すだけ・上書きしない）。★演出は出さない（checkQuests非呼び）＝派生値は読み時に再計算
  if(!authUser || _pulling) return; _pulling=true; const myUid=authUser.id;   // pull単一化。★捕捉uidを保持し、await後の書き込み前に再検証（in-flight中のログアウト/別アカ切替でローカルを汚さない）
  try{ const c=await sbClient();
    const { data:rows, error }=await c.from('sightings').select('*').eq('user_id',myUid);
    if(!authUser || authUser.id!==myUid) return;   // ★await中に別アカ/ログアウト→ローカルに書かない
    if(!error && Array.isArray(rows)){
      const local=USER.all(), have=new Set(local.map(r=>r.cid).filter(Boolean)), add=[];
      for(const row of rows){ if(row.client_id && !have.has(row.client_id)) add.push(rowToRec(row)); }
      if(add.length) USER.replaceAll(local.concat(add));
      rows.forEach(row=>{ if(row.client_id) SYNCED.add(row.client_id); }); saveSynced();   // cloudにある＝同期済み
    }
    const [se,qu,ba]=await Promise.all([
      c.from('user_seen').select('species_id').eq('user_id',myUid),
      c.from('user_quests').select('quest_id').eq('user_id',myUid),
      c.from('user_badges').select('badge_id').eq('user_id',myUid)]);
    if(!authUser || authUser.id!==myUid) return;   // ★状態書き込み前も再検証
    _syncingState=true;   // これらの save は再pushを誘発させない
    let ch=false;
    (se.data||[]).forEach(r=>{ STATESYNCED.add('s:'+r.species_id); if(!SEEN.has(r.species_id)){ SEEN.add(r.species_id); ch=true; } });   // cloudにある＝push済みとして記録（再pushしない）
    (qu.data||[]).forEach(r=>{ STATESYNCED.add('q:'+r.quest_id); if(!QDONE.has(r.quest_id)){ QDONE.add(r.quest_id); ch=true; } });
    (ba.data||[]).forEach(r=>{ STATESYNCED.add('b:'+r.badge_id); if(!BADGES.has(r.badge_id)){ BADGES.add(r.badge_id); ch=true; } });
    saveStateSynced();
    if(ch){ saveSeen(); saveQuests(); saveBadges(); }
    refreshAfterSync();
  }catch(e){} finally{ _syncingState=false; _pulling=false; }
}
function migrateAndPull(){   // 初回ログイン＝push(local→cloud)→pull(cloud→local union)。単一化・冪等
  if(!authUser) return Promise.resolve();
  if(_migrateP) return _migrateP;
  _migrateP=(async()=>{ try{ await pushAllSightings(); await pushState(); await syncPull(); uploadPendingPhotos(); }catch(e){} finally{ _migrateP=null; } })();
  return _migrateP;
}
function refreshAfterSync(){ try{ if(typeof updateDex==='function') updateDex(); if(document.getElementById('myDexModal')) openMyDex(true); if(typeof currentAnimal!=='undefined' && currentAnimal) updateSeenBtn(currentAnimal.id); }catch(e){} }   // openMyDex(true)=silent＝解析track/クエスト演出を出さない（同期由来の再描画）
// ── コミットE：写真を Storage(private) へ。パス ${uid}/${cid}.jpg・upsert:true で冪等。落ちてもテキスト同期は生きる。表示は署名URL遅延 ──
function dataURLtoBlob(durl){ try{ const i=durl.indexOf(','); const meta=durl.slice(0,i), b64=durl.slice(i+1); const mime=(meta.match(/data:([^;]+)/)||[])[1]||'image/jpeg';
  const bin=atob(b64), u=new Uint8Array(bin.length); for(let k=0;k<bin.length;k++)u[k]=bin.charCodeAt(k); return new Blob([u],{type:mime}); }catch(e){ return null; } }
async function uploadPhoto(rec){   // 1枚アップ→成功したら行の photo_path を更新＋ローカルにも記録。失敗は次の機会に再試行（rec.photo_path未設定のまま）
  if(!authUser || !rec || !rec.photo || !rec.cid || rec.photo_path) return;
  try{ const blob=dataURLtoBlob(rec.photo); if(!blob) return;
    const path=authUser.id+'/'+rec.cid+'.jpg', c=await sbClient();
    const { error }=await c.storage.from('sightings').upload(path, blob, { upsert:true, contentType:'image/jpeg' });
    if(error) return;
    await c.from('sightings').update({ photo_path:path }).eq('user_id',authUser.id).eq('client_id',rec.cid);
    rec.photo_path=path; USER.persist();
  }catch(e){}
}
async function inChunks(arr,n,fn){ for(let i=0;i<arr.length;i+=n){ await Promise.all(arr.slice(i,i+n).map(fn)); } }   // 小並列度（rate-limit回避）
async function uploadPendingPhotos(){ if(!authUser) return; try{ await inChunks(USER.all().filter(r=>r.photo&&r.cid&&!r.photo_path),4,uploadPhoto); }catch(e){} }
const _signedCache=new Map();
async function signedPhotoURL(path){   // private バケットは公開URL不可＝短命の署名URL（メモリキャッシュ・期限切れ再発行）
  if(!path || !authUser) return null;
  const hit=_signedCache.get(path); if(hit && hit.exp>Date.now()) return hit.url;
  try{ const c=await sbClient(); const { data, error }=await c.storage.from('sightings').createSignedUrl(path,3600);
    if(error||!data||!data.signedUrl) return null; _signedCache.set(path,{ url:data.signedUrl, exp:Date.now()+3300000 }); return data.signedUrl;
  }catch(e){ return null; }
}
async function hydratePhotos(root){   // 他端末からpullした記録（local dataURL無し・photo_pathあり）を、表示された行だけ署名URLで遅延表示
  if(!root) return; try{ await inChunks([...root.querySelectorAll('[data-pp]')],6,async el=>{ const url=await signedPhotoURL(el.getAttribute('data-pp')); if(url&&el.isConnected) el.innerHTML='<img src="'+url+'" alt="">'; }); }catch(e){}
}
...
function renderLegend(mode){
  ...
  if(mode==='overview'){
    ...
    title='📍 近くの生きもの';
    body=`<div class="lr"><span class="sw" style="background:#ff4d6d;box-shadow:none"></span>あなたの場所（中心）</div>
      <div class="lr"><span class="sw" style="background:transparent;border:1.5px dashed #34d8c6;box-shadow:none"></span>この範囲を探しています</div>
      <div class="lr" style="color:#ffcf6b"><span class="sw" style="background:#ffb02e;box-shadow:0 0 8px #ffb02e"></span>⚠ 絶滅危惧の生きもの</div>
      <div class="lr" style="color:var(--muted-2);font-size:11px">アイコンをタップで詳しく見る</div>`;
  } else {
    ...
    foot=`<div class="lf">🛰️ ボタンで、この生きものが<b>よく見つかる場所</b>を色の濃さで表示できます。</div>`;
  }
...
function markSeen(id){ if(!SEEN.has(id)){ SEEN.add(id); localStorage.setItem('biosphere_seen',JSON.stringify([...SEEN])); const ch=_chipPool.get(id); if(ch)ch.classList.add('is-seen'); updateDex(); celebrate(id); if(typeof scheduleStatePush==='function') scheduleStatePush(); } }   // プール要素を更新＝窓外でも再表示時に発見済み表示が保たれる
function saveBadges(){ try{ localStorage.setItem('biosphere_badges',JSON.stringify([...BADGES])); }catch(e){} if(typeof scheduleStatePush==='function') scheduleStatePush(); }
...
// 「図鑑」ピルから図鑑ドックを開いて検索にフォーカス（初見の一覧導線。以前はトーストを出すだけの空振りだった）
function openDexDock(){
  if(typeof closePanel==='function') closePanel();   // 近くパネル等が開いていると≤860pxでドックが隠れる＝先に閉じる（敵対レビュー確定）
  if(typeof buildChips==='function') buildChips();   // チップ構築を保証（モバイルはドック開で構築）
  const d=$('#dock'); if(d){ d.classList.add('open'); const t=$('#dockToggle'); if(t)t.setAttribute('aria-expanded','true'); if(matchMedia('(max-width:860px)').matches) d.scrollIntoView({behavior:'smooth',block:'nearest'}); }
  const s=$('#dockSearch'); if(s){ try{ s.focus({preventScroll:true}); }catch(e){ try{ s.focus(); }catch(_){} } }
}
...
// 種別チップ。「すべて」＝脊椎5クラス＋🦋昆虫を全部（ユーザー要望）。個別クラス（🦋虫含む）を選べばそのクラスだけに絞る。
...
// ① 表示数はユーザー可変。★一覧の行数＝地図アイコン数を同数に揃える（「表示した数分だけ」）。
//   実測(GBIF)：座標つき別種は 1000kmで~290 / 100kmで~180 / 10kmで~90、総観測点は数千。
//   →「別種」を優先で並べ、足りない分は同一種を別の実観測地点にも出して指定数まで埋める（fillRepeats）。
const NEARCAP_MIN=8, NEARCAP_DEF=24;
function capMax(r){ return r>=1000 ? 300 : r>=500 ? 200 : 100; }   // 表示数の上限（半径が広いほど多く）＝一覧・地図アイコン共通
...
let nearCap=nearCapWish;   // 近くモードの表示数（一覧の行数＝地図アイコン数の両方に効く・8〜半径依存）
const CREATURE_POOL=120;   // キャッシュに保存する候補種の上限（再訪の抽選母数）。1種あたりの保存点数は書込時に絞る（localStorage節約）。
...
// 1種の観測点群を「重心付近＋広がる数点」に間引く（散らし用の候補）。max=保持点数（repeatで散らす材料。既定5）。
function thinPoints(list,max){ max=max||5; if(!list||!list.length) return []; if(list.length<=max) return list.slice();
  const out=[medoid(list)]; while(out.length<max){ const p=spreadPoint(list,out); if(!p||out.some(q=>q[0]===p[0]&&q[1]===p[1])) break; out.push(p); } return out; }
...
// ★「すべて」は脊椎5クラス＋昆虫（🦋）も含める（ユーザー要望＝すべて＝全部出す）。個別クラスを選べば従来どおりそのクラスだけ。
const GBIF_ALL_GROUPS=GBIF_VERT_GROUPS.concat([{c:'Insecta',keys:[216],cap:12}]);
// 近くマーカー/一覧の取得グループ。種別を1つ以上選ぶとその選択クラスだけ（複数選択可）／未選択＝「すべて」は脊椎5＋昆虫。
function nearGroups(){
  if(!nearClasses.length) return GBIF_ALL_GROUPS;
...
let nearAvail=0;   // この範囲＋絞り込みで実際にいる別種数（facetで算出・capMaxまで）＝表示数インジケータの分母（例「100/50」の50）
...
let creatureMarkers=[], creatureTimer=null, creatureFillTimer=null, creaturesKey=null, threatToastShown=false, creatureGen=0, queryGen=0, _placedScreen=[];   // Gen=単調増加の世代トークン（同一丸め座標への素早い出戻りA→B→Aでも古いin-flightバッチを確実に破棄）／_placedScreen=配置済みマーカーの画面座標（真上のかぶりを避けて少しずらす用）
const JIT_MIN=24, JIT_STEP=20;   // マーカー同士の最小画面間隔(px)＝これ未満に重なる時だけ実観測点付近へずらす（40pxアイコンが真上に完全重複しない・多少の重なりは許容）
...
// GBIF（マーカーの絶滅危惧判定 resolveIucn 等）も throttle。表示数を最大300にしたので、cold cache では別種~290ぶんの
// species/match＋IUCN が一気に飛びうる→バースト/429を避けるため同時5でならす（dedupeで同一種は1本・成功はキャッシュ）。
let gbifQueue=[], gbifBusy=0;
function gbifEnqueue(fn){ gbifQueue.push(fn); pumpGbif(); }
function pumpGbif(){ while(gbifBusy<5 && gbifQueue.length){ const fn=gbifQueue.shift(); gbifBusy++; Promise.resolve(fn()).catch(()=>{}).finally(()=>{ gbifBusy--; setTimeout(pumpGbif,60); }); } }
...
function setNearPin(latRaw,lngRaw,label,radius){
  ...
  if(typeof renderLegend==='function') renderLegend('near');   // 近く専用の凡例に切替（種の多さ凡例の残留を防ぐ）
...
function queryNear(immediate){
  ...
  const capmx0=capMax(radius);
  if(cached&&cached.length){ nearAvail=Math.min(capmx0, cached.length);   // 再訪：キャッシュは全distinct（capMaxまで）を保存済み＝分母を即復元
    nearRows=cached.slice(0,nearCap).map(c=>({name:c.name,count:c.count,gcls:c.gcls,ocls:c.ocls||''})); renderNearList(); }   // 現 nearCap で頭切り（表示数を下げた直後に found>cap の矛盾表示を出さない）
  else { nearAvail=0; nearRows=[]; renderNearList('<div class="nearsum">🛰️ 周辺の記録を集めています…</div>'); }   // 未取得中は分母を「…」に
  clearTimeout(nearTimer);
  const run=()=>{
    const y2=new Date().getFullYear(), y1=y2-10;
    const capmx=capMax(radius);         // 表示数の上限＝availの上限（この範囲でこれ以上は表示対象にしない）
    ...
    const rebuild=()=>{   // 現在返っているクラスをラウンドロビンでインターリーブ（先頭からクラスが交互＝1クラスに偏って見えない）。全distinct（capMaxまで）を返す。
      ...
      for(let i=0;i<maxLen;i++) for(let gi=0;gi<groupRows.length;gi++){ const rr=groupRows[gi], c=rr&&rr[i]; if(!c) continue;
        ...
        merged.push(obj); }
      nearAvail=Math.min(capmx, merged.length);   // ★その範囲＋絞り込みにいる別種数（表示できる上限＝インジケータの分母「/50」）
      return merged;   // 表示行は呼び出し側で nearCap に切る（一覧件数は表示数コントロール連動）
    };
    groups.forEach((g,gi)=>{
      const isFish=g.c==='Fish';   // ① 魚グループは GBIF facet ＋ OBIS checklist を併合（外洋の魚も一覧に）
      // facetLimit は capMax まで＝「その範囲に何種いるか(avail)」を capMax まで数えられる（分母表示・並び替えが全件に効く）。
      const flim=capmx;
      const fp=isFish
      ...
      fp
        .then(rows=>{ if(stale()) return;
          groupRows[gi]=rows.slice(0,capmx).map(c=>({name:c.name,count:c.count,gcls:g.c,ocls:c.ocls||''}));   // 各クラスから capMax まで保持（avail集計と表示の母数）。ocls=OBIS由来の実クラス（クジラを魚扱いしない）
          if(!firstPainted){ const merged=rebuild(); if(merged.length){ nearRows=merged.slice(0,nearCap); renderNearList(); firstPainted=true; } }   // 最速クラスで一覧を即出す
        })
        .catch(()=>{})
        .finally(()=>{ settled++; if(settled!==groups.length || stale()) return;   // 全クラス出そろい＝確定描画＋キャッシュ
          const merged=rebuild();
          if(!merged.length){ noneMsg(); }
          else { nearCacheSet(k,merged.map(c=>({name:c.name,count:c.count,gcls:c.gcls,ocls:c.ocls||''}))); nearRows=merged.slice(0,nearCap); renderNearList(); }   // キャッシュは全distinct（再訪でavail即復元）／表示は nearCap
        });
    });
  };
...
function nearControlsHTML(){
  ...
  // 表示数コントロール：PC=スライダー／スマホ=チップ。右に「いま地図に出ているアイコン数 / この範囲にいる種数」を表示。
  //   分母(nearAvail)はその範囲＋絞り込みにいる別種数（capMaxまで）。分子は表示アイコン数（種数を上げると同じ種も別地点に出て種数を超えられる＝例「100/50」）。
...
function applyNearFilter(){
  ...
  const thChip=(thCount>0||nearThreatOnly)?`<button class="thsum${nearThreatOnly?' on':''}" onclick="toggleNearThreat()">⚠ 近くの絶滅危惧 <b>${thCount}</b>種${nearThreatOnly?'（解除）':''}</button>`:'';
  let head;
...
// 表示数コントロールの「🗺️ N」＝いま地図に出ているアイコン数（指定した数分だけ出ているかの確認・任意要素）。
function updateCreatureCount(){ const el=document.getElementById('capMapCount'); if(!el) return;
  const n=el.querySelector('.capn'), a=el.querySelector('b');
  if(n) n.textContent=creatureMarkers.length;               // 分子＝いま地図に出ているアイコン数
  if(a) a.textContent=nearAvail>0?nearAvail:'…';            // 分母＝この範囲・絞り込みにいる種数（未取得中は…）
}
function removeCreatures(){ creatureMarkers.forEach(m=>{try{m.remove();}catch(e){}}); creatureMarkers=[]; _placedScreen=[]; updateCreatureCount(); }
...
// 重み付き1件抽出（非破壊＝同じ種を何度も選べる・repeat配置用）。weight=頻度n。
function weightedPick(pool){ if(!pool||!pool.length) return null; let tot=0; for(const it of pool) tot+=Math.max(1,it.n||1);
  let r=Math.random()*tot; for(let i=0;i<pool.length;i++){ r-=Math.max(1,pool[i].n||1); if(r<=0) return pool[i]; } return pool[pool.length-1]; }
// 候補点群から「既存マーカーから最も遠い点」を取り出し、その点を候補から除く（次は別地点＝同一点の重複配置を防ぐ）。
function spreadTake(pts, placed){ const c=spreadPoint(pts, placed); if(!c) return null;
  const i=pts.findIndex(p=>p[0]===c[0]&&p[1]===c[1]); if(i>=0) pts.splice(i,1); return c; }
// Promiseにタイムアウト（遅い外部API＝OBISの巨大bbox等で群が固まって最終処理が走らないのを防ぐ）。超過時は fb を返す。
function pTimeout(p, ms, fb){ return Promise.race([p, new Promise(res=>setTimeout(()=>res(fb), ms))]); }
// occurrence群→種ごとの{頻度・観測点・クラス}に集計（o.species無しも scientificName を二名化して拾う＝取りこぼし減）。
function tallyOcc(res, gcls){
  const freq={}, pts={}, cls={};
...
function loadNearCreatures(lat,lng,radius,immediate){
  ...
  const cap=nearCap;           // ★アイコン数＝一覧の表示数（表示した数分だけ・cached/run 両経路で使用）
  clearTimeout(creatureTimer); clearTimeout(creatureFillTimer);
  const POOL_PTS=24;           // 1種あたり保持する観測点数（repeatで別地点に散らす材料）
  const shown=new Set();       // 既に「別種」として置いた種（小文字）。repeat配置はshownを無視して同一種を別地点に出す。
  const speciesPool=new Map(); // sci(lower)→{sci,cls,n,pts[],cachePts[]}＝別種配置＋repeat＋キャッシュの母数。pts は使うたび減る／cachePts は不変（保存用）。
  const addSpecies=(sci,cls,n,pts)=>{ const kk=sci.toLowerCase(); const e=speciesPool.get(kk);
    if(e){ e.n=Math.max(e.n,n); for(const p of pts){ if(e.pts.length>=POOL_PTS) break; e.pts.push(p); } for(const p of pts){ if(e.cachePts.length>=6) break; e.cachePts.push(p); } }
    else speciesPool.set(kk,{sci,cls,n,pts:pts.slice(0,POOL_PTS),cachePts:pts.slice(0,6)}); };
  const placedOf=()=>creatureMarkers.map(m=>{const o=m.getLngLat();return [o.lng,o.lat];});   // 既存マーカー座標（散らしの基準）
  // ⑤ 直近に見た地点はキャッシュのプールから即マーカー化（往復ゼロ＋毎回ちがう顔ぶれ）。裏で最新を取得しプール更新。
  const cached=creatureCacheGet(k);
  if(cached&&cached.length){ cached.filter(p=>p&&p.sci&&Array.isArray(p.pts)&&p.pts.length).forEach(p=>addSpecies(p.sci,p.cls,p.n,p.pts));
    const placed=[], cp=[];
    for(const e of weightedSample([...speciesPool.values()], cap)){ const c=spreadPoint(e.pts, placed); if(!c) continue; cp.push({sci:e.sci,cls:e.cls,n:e.n,c}); placed.push(c); }
  ...
  const run=()=>{
    ...
    let widened=false;
    // ★段階描画：クラス別 occurrence を Promise.all で束ねず、返ってきたクラスから即「別種」を配置（最速クラスで初アイコン・鳥待ちしない）。
    const single=groups.length===1;
    const occS=Math.min(300,Math.max(140,cap+40));                                  // 単一クラス選択時の取得件数
    const perGrp=single?occS:Math.min(300,Math.max(90,Math.round(cap*1.2)));        // すべて時の各群取得件数（capが高いほど厚く＝別種と点を確保。既定cap24なら90＝従来と同等）
    const distinctBudget=single?cap:Math.ceil(cap/groups.length)+4;                 // 段階描画の別種は各群 均等配分（鳥偏重を防ぐ）。余りは下の fillRepeats で。
    // ★指定数まで埋める：まず未表示の別種を全部（多様性優先）→それでも足りなければ同一種の別の実観測地点（repeat）。Q1=repeats・Q2=strict（間引かない）。
    const fillRepeats=()=>{ let room=cap-creatureMarkers.length; if(room<=0) return;
      const placed=placedOf(); const add=[];
      const fresh=[...speciesPool.values()].filter(e=>!shown.has(e.sci.toLowerCase()) && e.pts&&e.pts.length);   // 段階描画の配分で置けなかった別種
      for(const e of weightedSample(fresh, room)){ if(room<=0) break; const c=spreadTake(e.pts, placed); if(!c) continue; add.push({sci:e.sci,cls:e.cls,n:e.n,c}); placed.push(c); shown.add(e.sci.toLowerCase()); room--; }
      let pool=[...speciesPool.values()].filter(e=>e.pts&&e.pts.length); let guard=0;   // repeat（同一種の別地点）
      while(room>0 && pool.length && guard<cap*30){ guard++;
        const e=weightedPick(pool); if(!e) break;                                   // 頻度重み＝多い種ほど repeat 出やすい（実際その辺に何羽もいる＝リアル）
        const c=spreadTake(e.pts, placed);
        if(!e.pts.length){ const i=pool.indexOf(e); if(i>=0) pool.splice(i,1); }     // 点を使い切った種はプールから外す
        if(!c) continue;
        add.push({sci:e.sci,cls:e.cls,n:e.n,c}); placed.push(c); room--; }
      if(add.length) spawnCreatures(add,true,gen);
    };
    // ★最終処理（fillRepeats＋キャッシュ）＝一度だけ。全群settle か、遅い群のフォールバックタイマーのどちらか早い方で走る。
    let finalized=false;
    const finalize=()=>{ if(finalized||stale()) return; finalized=true; clearTimeout(creatureFillTimer);
      fillRepeats();   // 別種で足りない分を同一種の別地点で cap まで埋める
      if(speciesPool.size) creatureCacheSet(k, [...speciesPool.values()].slice(0,CREATURE_POOL).map(e=>({sci:e.sci,cls:e.cls,n:e.n,pts:(e.cachePts&&e.cachePts.length?e.cachePts:e.pts||[]).slice(0,6)})));   // 種プールをキャッシュ（点は6まで＝localStorage節約・cachePtsは配置で消費されない不変コピー）
    };
    const grab=(r)=>{
      let settled=0;
      groups.forEach(g=>{
        const isFish=g.c==='Fish';   // ① 魚は GBIF(縛り緩和)＋OBIS(海洋)を併合＝外洋/海でもマーカーが出る。OBISは巨大bboxで遅い→6秒で打ち切り（魚GBIF分は活かす）。
        const op=isFish
          ? Promise.all([gbifOccByGroup(lat,lng,r,g.keys,single?Math.min(240,occS):perGrp,y1,y2,false), pTimeout(obisOcc(lat,lng,r,single?220:140),6000,[])]).then(([a,b])=>a.concat(b))
          : gbifOccByGroup(lat,lng,r,g.keys,perGrp,y1,y2,true);
        op
          .then(res=>{ if(stale()) return;
            const t=tallyOcc(res, g.c);
            Object.keys(t.freq).forEach(sp=>{ const cls=CLASS_EMOJI[t.cls[sp]]?t.cls[sp]:g.c; addSpecies(sp,cls,t.freq[sp],thinPoints(t.pts[sp],POOL_PTS)); });   // 種プールへ蓄積（repeat/キャッシュの母数）
            // 別種優先で新規種を配置（このグループ配分＝distinctBudget まで・段階描画）
            const gRoom=Math.min(cap-creatureMarkers.length, distinctBudget); if(gRoom>0){
              const cand=[...speciesPool.values()].filter(e=>t.freq[e.sci]!=null && !shown.has(e.sci.toLowerCase()) && e.pts.length);   // このグループの応答にある未表示種
              const placed=placedOf(); const add=[];
              for(const e of weightedSample(cand, gRoom)){ if(add.length>=gRoom) break; const c=spreadTake(e.pts, placed); if(!c) continue; add.push({sci:e.sci,cls:e.cls,n:e.n,c}); placed.push(c); shown.add(e.sci.toLowerCase()); }
          ...
          .finally(()=>{ settled++;
            ...
            //   8秒フォールバックで既に finalize 済みでも、widen分を fill/キャッシュへ反映するため finalized を解除して再 finalize させる。
            if(!widened && !stale() && creatureMarkers.length<4 && r<80){ widened=true; finalized=false; grab(Math.min(r*3,100)); return; }
            finalize();
          });
      });
    };
    grab(radius);
    // ★フォールバック：どれかの群が遅い/固まっても、8秒後には現状のプールで fillRepeats を確実に走らせる（指定数まで埋める）。
    creatureFillTimer=setTimeout(()=>{ if(!stale()) finalize(); }, 8000);
  };
...
function spawnCreatures(list,append,gen){
  ...
  // ★指定数は厳密に出す（間引かない）が、真上に完全重複するアイコンだけは実観測点付近へ少しずらす（_placedScreen＋フィボナッチ螺旋）。
  list.forEach((it,idx)=>{
    let c=it.c, sp=null; try{ sp=map.project(c); }catch(e){}
    if(sp){   // 既存マーカーと JIT_MIN 未満で重なるなら、螺旋状に少しずらした点へ（unprojectで実観測点付近の座標に戻す）
      let cur={x:sp.x,y:sp.y}, t=0;
      const hit=p=>_placedScreen.some(q=>{const dx=p.x-q.x,dy=p.y-q.y;return dx*dx+dy*dy < JIT_MIN*JIT_MIN;});
      while(hit(cur) && t<16){ t++; const a=t*2.399963267, r=JIT_STEP*Math.sqrt(t); cur={x:sp.x+Math.cos(a)*r, y:sp.y+Math.sin(a)*r}; }
      if(t>0){ try{ const ll=map.unproject([cur.x,cur.y]); c=[ll.lng,ll.lat]; }catch(e){} }   // 重なった時だけ座標を微調整
      _placedScreen.push(cur);
    }
    const el=document.createElement('div'); el.className='cmk-wrap'; el.style.setProperty('--d',Math.min((base+idx)*30,1400)+'ms');   // 出現stagger（多数でも約1.4秒で出そろうよう頭打ち）＝wrapに置き .cmk pop と .cmk-lab peek に継承
    ...
    const lab=document.createElement('div'); lab.className='cmk-lab'; lab.innerHTML=cja?`<b>${esc(cja)}</b>`:`<i>${esc(it.sci)}</i>`;   // 名前ラベル（自動ピークは先頭数体だけ＝下のpeek。残りはタップ/ホバーで表示）
    if(creatureMarkers.length<6) lab.classList.add('peek');   // 全同時ピークをやめ団子を防ぐ
    el.appendChild(bub); el.appendChild(lab);
    el.addEventListener('click',(e)=>{ e.stopPropagation(); document.querySelectorAll('.cmk-wrap.sel').forEach(x=>x.classList.remove('sel')); el.classList.add('sel'); openCreature(it.sci,it.n,it.cls); });
    let mk; try{ mk=new maplibregl.Marker({element:el,anchor:'center'}).setLngLat(c).addTo(map); }catch(e){ return; }
    ...
    // 近くの絶滅危惧種を強調（「近所にコレ!?」）＋ラベルにも保全状況。★throttle（gbifEnqueue・同時5）で最多300時のGBIFバーストを避ける。
    gbifEnqueue(async()=>{ if(myGen!==creatureGen||creaturesKey!==myKey||!bub.isConnected) return; const code=await resolveIucn(it.sci);
      if(myGen!==creatureGen||creaturesKey!==myKey||!bub.isConnected) return; if(code&&THREAT_CATS.has(code)&&RARITY[code]){ bub.classList.add('threat'); bub.style.setProperty('--tc',RARITY[code].color); lab.classList.add('th'); lab.insertAdjacentHTML('afterbegin',`<span class="w">⚠${code}</span> `);
      if(!threatToastShown){ const nm=inatGet(it.sci); const ja=cja||(nm&&nm.ja); if(ja){ threatToastShown=true; toast('⚠','近くに絶滅危惧の「'+esc(ja)+'」がいるかも',3800); } } } });   // 和名はiNat由来＝escでXSS防止／「近所にコレ!?」を1地点1回（敵対レビュー確定）
  });
  updateCreatureCount();   // 表示数コントロールの「🗺️ N」を今のアイコン数に更新
}
...
function fillOnboardSpecies(){
  ...
  const cnt=document.getElementById('wcmCount'); if(cnt&&ANIMALS.length) cnt.textContent=ANIMALS.length.toLocaleString()+'種';   // 種数は実データ(ANIMALS.length)から上書き＝ハードコードの古い数値を防ぐ
...
async function buildShareCanvas(o){
  ...
  if(o.figure){
    ...
    x.fillStyle='#cdd9df';x.font='30px sans-serif';x.fillText('周辺 半径'+o.radius+'km ・ 生きもの '+o.total+'種',58,286);
...
async function shareNearCard(btn){
  ...
  try{
    ...
    const text=`📍${place}の近くにいる生きもの ${nearRows.length}種`+(threatened?`（うち絶滅危惧${threatened}種）`:'')+' #Faunaut';
...
$('#gbifBtn').addEventListener('click',()=>{ gbifOn=!gbifOn;
  const w=isWideBasis(currentAnimal);   // 魚は標本・調査記録も含む＝「観測」でなく「記録」に（カード文言と揃える）
  toast('🛰️', currentAnimal?(gbifOn?(w?'記録の多い場所を色で表示：ON':'観測の多い場所を色で表示：ON'):'色分け：OFF（手描き分布にもどす）'):'動物を選ぶと、よく見つかる場所を表示できます',1900);   // 概観は種未選択＝no-opなのでヒントに（敵対レビュー確定）
...
$('#dex').addEventListener('click',()=>{ openDexDock(); });
