...
function sightingRow(rec){ return { user_id:authUser.id, client_id:rec.cid, species_id:rec.id, species_sci:rec.sci||null,
  ...
  place_label:rec.place||null, confidence:rec.confidence||null, note:rec.note||null, source:rec.source||'manual' }; }   // 写真(photo_path)はコミットEで
let _pushing=false;
async function pushSighting(rec){   // 1件をクラウドへ（fire-and-forget・冪等upsert）。ゲスト/未ログインは即no-op
  ...
  try{
    ...
    if(!error){ SYNCED.add(rec.cid); saveSynced(); }
  }catch(e){}
}
...
function migrateAndPull(){   // 初回ログイン＝push(local→cloud)→pull(cloud→local union)。単一化・冪等
  ...
  _migrateP=(async()=>{ try{ await pushAllSightings(); await pushState(); await syncPull(); }catch(e){} finally{ _migrateP=null; } })();
  return _migrateP;
}
