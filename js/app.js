...
function onAuth(ev, session){   // ログイン状態が変わったらUIを更新＋同期をトリガ（local-firstの背景同期）
  ...
  if(ev==='SIGNED_OUT'){ authUser=null; SYNCED.clear(); saveSynced(); updateAuthUI(); }   // 別アカ混線防止＝synced集合をクリア（次アカで全件再push）
}
...
async function pushSighting(rec){   // 1件をクラウドへ（fire-and-forget・冪等upsert）。ゲスト/未ログインは即no-op
  ...
  try{
    if(!rec.cid){ rec.cid=genCid(); USER.persist(); }
...
async function pushAllSightings(){   // 未同期のローカル記録を全部push（ログイン時・online復帰・タブ復帰）。冪等（unique(user_id,client_id)+upsert）
  ...
  try{
    USER.ensureCids();   // 既存記録にcidをbackfill（1回だけ・重複防止の要）
    const c=await sbClient();
...
addEventListener('storage', e=>{ if(!e.key||e.key.indexOf('biosphere_')!==0) return; try{
  ...
  else if(e.key==='biosphere_seen'){ const s=JSON.parse(e.newValue||'[]'); SEEN.clear(); s.forEach(x=>SEEN.add(x)); if(typeof updateDex==='function')updateDex(); }
  else if(e.key==='biosphere_quests'){ const s=JSON.parse(e.newValue||'[]'); QDONE.clear(); s.forEach(x=>QDONE.add(x)); }
  else if(e.key==='biosphere_badges'){ const s=JSON.parse(e.newValue||'[]'); BADGES.clear(); s.forEach(x=>BADGES.add(x)); }
}catch(_){} });
...
async function pushState(){   // ローカルの seen/quests/badges＋profile をクラウドへ（冪等・ignoreDuplicates）
  if(!authUser) return;
  try{ const c=await sbClient(), uid=authUser.id, jobs=[];
    if(SEEN.size)   jobs.push(c.from('user_seen').upsert([...SEEN].map(species_id=>({user_id:uid,species_id})),{onConflict:'user_id,species_id',ignoreDuplicates:true}));
    if(QDONE.size)  jobs.push(c.from('user_quests').upsert([...QDONE].map(quest_id=>({user_id:uid,quest_id})),{onConflict:'user_id,quest_id',ignoreDuplicates:true}));
    if(BADGES.size) jobs.push(c.from('user_badges').upsert([...BADGES].map(badge_id=>({user_id:uid,badge_id})),{onConflict:'user_id,badge_id',ignoreDuplicates:true}));
    jobs.push(c.from('profiles').upsert({id:uid,display_name:accountName()||null,updated_at:new Date().toISOString()},{onConflict:'id'}));
    await Promise.all(jobs);
  }catch(e){}
}
async function syncPull(){   // cloud→local を union（欠けを足すだけ・上書きしない）。★演出は出さない（checkQuests非呼び）＝派生値は読み時に再計算
  if(!authUser || _pulling) return; _pulling=true;   // pull は単一化（多重発火のinterleave防止）
  try{ const c=await sbClient(), uid=authUser.id;
    const { data:rows, error }=await c.from('sightings').select('*').eq('user_id',uid);
    ...
    const [se,qu,ba]=await Promise.all([
      c.from('user_seen').select('species_id').eq('user_id',uid),
      c.from('user_quests').select('quest_id').eq('user_id',uid),
      c.from('user_badges').select('badge_id').eq('user_id',uid)]);
    _syncingState=true;   // これらの save は再pushを誘発させない
    let ch=false;
    (se.data||[]).forEach(r=>{ if(!SEEN.has(r.species_id)){ SEEN.add(r.species_id); ch=true; } });
    (qu.data||[]).forEach(r=>{ if(!QDONE.has(r.quest_id)){ QDONE.add(r.quest_id); ch=true; } });
    (ba.data||[]).forEach(r=>{ if(!BADGES.has(r.badge_id)){ BADGES.add(r.badge_id); ch=true; } });
...
function refreshAfterSync(){ try{ if(typeof updateDex==='function') updateDex(); if(document.getElementById('myDexModal')) openMyDex(); if(typeof currentAnimal!=='undefined' && currentAnimal) updateSeenBtn(currentAnimal.id); }catch(e){} }
...
async function uploadPendingPhotos(){ if(!authUser) return; try{ for(const r of USER.all().filter(r=>r.photo&&r.cid&&!r.photo_path)) await uploadPhoto(r); }catch(e){} }
...
async function hydratePhotos(root){   // 他端末からpullした記録（local dataURL無し・photo_pathあり）を、表示された行だけ署名URLで遅延表示
  if(!root) return; try{ for(const el of root.querySelectorAll('[data-pp]')){ const url=await signedPhotoURL(el.getAttribute('data-pp')); if(url) el.innerHTML='<img src="'+url+'" alt="">'; } }catch(e){}
}
